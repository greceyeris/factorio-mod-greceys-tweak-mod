...
data:extend{
    ...
    {
        type = "bool-setting",
        name = "enable-automatically-researches-worker-robots-speed-technologies-at-game-start",
        setting_type = "startup",
        default_value = false,
...
if mod_enabled("space-age") then
    -- 修复模组的问题
    -- 修复模组 greceys-essentials-modpack 的问题
    if not mod_enabled("greceys-essentials-modpack") then
        -- startup-settings
        data:extend{
            -- a[enable]-e[entity]
            {
...
if mod_enabled("recycler") then
    ...
    data:extend{
        ...
        {
            type = "bool-setting",
            name = "enable-remove-recycling-recipes-whose-names-end-in-recycling",
            setting_type = "startup",
            default_value = false,
            order = "a[enable]-h[recipe]"
        }
